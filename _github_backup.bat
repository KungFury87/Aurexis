...
set "REPO_DIR=%~dp0"
set "BRANCH=backup/v1-substrate-candidate-20260414-000001"
set "TAG=backup-v1-substrate-candidate-20260414-000001"
set "LOG=%REPO_DIR%_backup_log.txt"
set "GCM_GITHUBAUTHMODE=device"

echo ============================================ > "%LOG%"
echo Aurexis Core GitHub Backup >> "%LOG%"
echo Pytest Surface Green Pass — V1 Substrate Candidate — 51 bridges >> "%LOG%"
echo %date% %time% >> "%LOG%"
echo ============================================ >> "%LOG%"
...
if exist ".git" (
    ...
    REM Commit
    echo Committing... >> "%LOG%"
    git commit -m "Pytest Surface Green Pass — V1 Substrate Candidate — 51 bridges, 6358 assertions, 61 runners, 67 source modules, handoff-ready" >> "%LOG%" 2>&1
    ...
    git tag -f "%TAG%" -m "Backup: Pytest Surface Green Pass — V1 Substrate Candidate — 51 bridges, 6358 assertions, 61 runners, 67 source modules, 9 branches, handoff-ready" >> "%LOG%" 2>&1
