...
REM Temporal Branch COMPLETE-ENOUGH — 28 bridges + capstone (38 runners)

set "REPO_DIR=%~dp0"
set "BRANCH=backup/v1-substrate-candidate-20260413-temporal-complete"
set "TAG=backup-v1-substrate-candidate-20260413-temporal-complete"
set "LOG=%REPO_DIR%_backup_log.txt"
set "GCM_GITHUBAUTHMODE=device"

echo ============================================ > "%LOG%"
echo Aurexis Core GitHub Backup >> "%LOG%"
echo Temporal Branch COMPLETE-ENOUGH — 28 bridges + capstone >> "%LOG%"
echo %date% %time% >> "%LOG%"
echo ============================================ >> "%LOG%"
...
if exist ".git" (
    ...
    REM Commit
    echo Committing... >> "%LOG%"
    git commit -m "Temporal Branch COMPLETE-ENOUGH — 28 bridges, 4835 assertions, 38 runners, 40 modules, capstone verified" >> "%LOG%" 2>&1
    ...
    git tag -f "%TAG%" -m "Backup: V1 Substrate Candidate — Temporal Branch COMPLETE-ENOUGH (28 bridges, 10 temporal, capstone verified)" >> "%LOG%" 2>&1
