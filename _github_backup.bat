...
REM Temporal Payload Contract Bridge V1 (25th bridge)

set "REPO_DIR=%~dp0"
set "BRANCH=backup/v1-substrate-candidate-20260413-tpcontract"
set "TAG=backup-v1-substrate-candidate-20260413-tpcontract"
set "LOG=%REPO_DIR%_backup_log.txt"
set "GCM_GITHUBAUTHMODE=device"

echo ============================================ > "%LOG%"
echo Aurexis Core GitHub Backup >> "%LOG%"
echo Temporal Payload Contract Bridge V1 >> "%LOG%"
echo %date% %time% >> "%LOG%"
echo ============================================ >> "%LOG%"
...
if exist ".git" (
    ...
    REM Commit
    echo Committing... >> "%LOG%"
    git commit -m "Temporal Payload Contract Bridge V1 — 25th bridge milestone, 4480 assertions, 35 runners, 37 modules" >> "%LOG%" 2>&1
    ...
    git tag -f "%TAG%" -m "Backup: V1 Substrate Candidate with Temporal Payload Contract Bridge (25th bridge)" >> "%LOG%" 2>&1
