...
REM Combined RS+CC Temporal Fusion Bridge V1 (24th bridge)

set "REPO_DIR=%~dp0"
set "BRANCH=backup/v1-substrate-candidate-20260413-fusion"
set "TAG=backup-v1-substrate-candidate-20260413-fusion"
set "LOG=%REPO_DIR%_backup_log.txt"
set "GCM_GITHUBAUTHMODE=device"

echo ============================================ > "%LOG%"
echo Aurexis Core GitHub Backup >> "%LOG%"
echo Combined RS+CC Temporal Fusion Bridge V1 >> "%LOG%"
echo %date% %time% >> "%LOG%"
echo ============================================ >> "%LOG%"
...
if exist ".git" (
    ...
    REM Commit
    echo Committing... >> "%LOG%"
    git commit -m "Combined RS+CC Temporal Fusion Bridge V1 — 24th bridge milestone, 4347 assertions, 34 runners, 36 modules" >> "%LOG%" 2>&1
    ...
    git tag -f "%TAG%" -m "Backup: V1 Substrate Candidate with Combined RS+CC Temporal Fusion Bridge (24th bridge)" >> "%LOG%" 2>&1
