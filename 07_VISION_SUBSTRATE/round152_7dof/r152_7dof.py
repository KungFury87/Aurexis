"""R152 - 7-DOF Phase 4 training (translation + rotation + scale).

7 params: (tx, ty, tz, rx, ry, rz, s)
Target: (0, 0, 0, 0, 0, 0, 1) — identity transform.
Init:   (1, 0.5, 0, 0.2, 0.2, 0.2, 1.1) — off in all axes.

Init dist in 7D: sqrt(1+0.25+0+0.04+0.04+0.04+0.01) = sqrt(1.38) = 1.175.

Same Adam+lr-decay schedule as R151:
  iters 1-10: lr=0.1, 11-15: 0.05, 16-20: 0.025, 21-25: 0.0125, 26-30: 0.00625

Same MV alpha=0.20, image_size=192, eps=0.05.
"""
...
OUT = Path('/tmp/round152_7dof'); OUT.mkdir(exist_ok=True)
...
def apply_7dof(field, params):
    """Apply 7-DOF transform to a phoxel field. params = (tx, ty, tz, rx, ry, rz, s)."""
...
print(f"R152 7-DOF: init={INIT}, target={TARGET}")
init_dist_7d = float(np.linalg.norm(INIT - TARGET))
print(f"7D init distance from target: {init_dist_7d:.4f}")
...
pn = rt.installed()
# Targets are rendered at TARGET params (identity)
...
for chunk_iter in range(N_ITERS_THIS_CHUNK):
    ...
    history.append((t, params.tolist(), d7, ti, pi, si, ji, lr_t))
    print(f"  it{t} d7D={d7:.4f} lr={lr_t:.4f} ph={pi:.4f} J={ji:.3f}")
