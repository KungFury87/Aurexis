"""R151 - extend R150 to 30 iters with continued lr halvings.

Schedule:
  iters 1-10:  lr=0.1   (warmup)
  iters 11-15: lr=0.05
...
OUT = Path('/tmp/round151_extended'); OUT.mkdir(exist_ok=True)
...
print(f"R151: Adam + extended lr-decay schedule, alpha={ALPHA}, image_size={IMG}, fixed eps={EPS}")
...
for chunk_iter in range(N_ITERS_THIS_CHUNK):
    ...
    print(f"  it{t} d={d:.4f} lr={lr_t:.4f} ph={pi:.4f} J={ji:.3f}")
